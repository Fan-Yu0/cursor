...
class CardKeyVerifier:
    def __init__(self):
        self.api_url = "http://111.229.80.250:8910/api/cardkey/consume"

    def verify_key(self, key_code: str) -> bool:
        """
        验证卡密
        ...
        try:
            ...
            # 构建带参数的URL
            url = f"{self.api_url}?keyCode={key_code}"
            ...
            if response.status_code == 200:
                ...
                if result.get("success") == True:  # 假设0表示成功
                    ...
                    logging.error(f"{EMOJI['ERROR']} 卡密验证失败: {error_msg}")
            else:
                logging.error(f"{EMOJI['ERROR']} 服务器响应错误: {response.status_code}")

        except requests.exceptions.RequestException as e:
            logging.error(f"{EMOJI['ERROR']} 网络请求失败: {str(e)}")
        except Exception as e:
            logging.error(f"{EMOJI['ERROR']} 验证过程出错: {str(e)}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 验证卡密
        if not verifier.verify_key(key):
            logging.error("卡密验证失败")
            sys.exit(1)
